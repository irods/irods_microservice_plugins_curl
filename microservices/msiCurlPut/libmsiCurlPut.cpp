...
int msiCurlPut(msParam_t* msp_in_str_url, msParam_t* msp_in_kvp_options, msParam_t* msp_in_kvp_curl_options, msParam_t* msp_out_str_response, ruleExecInfo_t* rei) {
    ...
    if (msp_in_kvp_curl_options == nullptr) {
        rodsLog(LOG_ERROR, "msiCurlPut: msp_in_kvp_curl_options is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    if (msp_in_kvp_curl_options->type == nullptr) {
        rodsLog(LOG_ERROR, "msiCurlPut: msp_in_kvp_curl_options->type is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    if (strcmp(msp_in_kvp_curl_options->type, KeyValPair_MS_T)) {
        rodsLog(LOG_ERROR, "msiCurlPut: second argument should be KeyValPair_MS_T, was [%s]", msp_in_kvp_curl_options->type);
        return USER_PARAM_TYPE_ERR;
    }
    ...
    irods::error res = myCurl.put(static_cast<char*>(msp_in_str_url->inOutStruct), static_cast<keyValPair_t*>(msp_in_kvp_options->inOutStruct), static_cast<keyValPair_t*>(msp_in_kvp_options->inOutStruct), reinterpret_cast<char**>(&msp_out_str_response->inOutStruct));
    return res.code();
}
